...
function buscarClima(e){
    ...
    if (ciudad === ''|| pais === '') {
        ...
        mostrarError('Ambos campos son obligados');
...
function consultaAPI(ciudad,pais){
    ...
    const appId = 'fd8639c99aaa94e189723867d8103380';
...
function mostrarClima(datos) {
    ...
    //formatear el clima c
console.log(datos)
    const {name, main: {temp, temp_max, temp_min}} = datos;
    // conversion de kelvin a centigrados
    const grados = parseInt( temp - 273.15);
    const min = kelvinCentigrados(temp_max);
    const max = kelvinCentigrados(temp_min);
